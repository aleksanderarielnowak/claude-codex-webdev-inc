...
- **2026-07-13 · Codex GPT-5.6 „Sol" + tuning effort do trudności · REGUŁA:** delegacje do Codexa (autorstwo `_elementor_data`/PHP/JSON, fetch/research, obrazy, headless wp-admin, bulk) idą na model **`gpt-5.6-sol`** (nigdy starszy), a reasoning effort dobierasz do zadania: `low` mechaniczne · `medium` standard (sekcja/podstrona/bulk) · `high` multi-plik/debug/headless/pierwszy nieznany stack · `xhigh` correctness-critical (REVIEW, root-cause, złożony refactor). Bias w dół, eskaluj retry. Źródło doktryny + flagi: `claude-codex-synapse` SKILL „Model & effort".
