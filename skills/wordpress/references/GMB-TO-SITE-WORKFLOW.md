...
- **Build-mode:** zawsze **NATYWNY Elementor**. Strony powstają z natywnych widgetów Elementora, header/footer przez HFE, formularze przez CF7, a dodatkowe elementy przez Happy Addons / Essential Addons.
...
- **Brak realnej marki (demo/portfolio/fictional rebrand):** przed projektowaniem uruchom skill `branding` (`../../branding/SKILL.md`) i zapisz `brand.json` albo `brand-spec.md` z nazwą, tagline, paletą, fontami, logo concept i tonem. Ten artefakt zasila design; nie zastępuje decyzji o stacku.
...
> v2: Codex AUTORUJE pliki natywnego buildera na dysk (z placeholderami `__IMG_*__`/`__CF7__`/`__MAP__`), CLAUDE APLIKUJE przez wp-cli (`wp eval-file build-<slug>-native.php`). Weryfikacja per etap: NAJPIERW HTTP strukturalnie, wizualnie na końcu/batch.
**Etap A — Fundament:** Claude przez wp-cli: motyw + wtyczki + header/footer (HFE, NIE hack) + Kit + puste strony. → weryfikacja wizualna.
**Etap B — Strony:** Codex autoruje `build-<slug>-native.php` na `native-lib.php` + `native.css` → Claude aplikuje `wp eval-file` → `wp elementor flush-css` / clear cache → weryfikacja (HTTP + screeny).
...
|---|---|---|---|
| **NATYWNY Elementor** | `gmb-workflow/NATIVE-ELEMENTOR-BUILD.md` + `gmb-workflow/assets/native/` | zawsze dla buildów stron w tym workflow | klient edytuje sekcje, teksty, obrazy i karty w Elementorze; wymagany dedykowany `native.css` |

To jedyny wspierany sposób budowy stron w tym repo.
...
**6.2A NATYWNY Elementor** — `gmb-workflow/NATIVE-ELEMENTOR-BUILD.md` + `gmb-workflow/assets/native/`. Workflow używa natywnych widgetów Elementora (`heading/text/button/icon-box/icon-list/image/gallery/accordion`) i wspólnego `native.css`, żeby klient mógł edytować stronę wizualnie.
...
- **`assets/`** — gotowce: `assets/native/` (helpery, `native.css`, przykład buildera, Kit) oraz `mu-kw-aktualnosci.php` (shortcode Aktualności).
