...
Kroki:
1. **Wczytaj protokół:** `skills/wordpress/references/METODA-BUDOWY-SEKCJAMI.md` (metoda nadrzędna — budujesz sekcjami, builder przed HTML-em), potem `skills/wordpress/SKILL.md` + master `skills/wordpress/references/GMB-TO-SITE-WORKFLOW.md`. Wstrzykuj guardraile z `LEARNINGS.md` do KAŻDEGO specu Codexa (sekcja „Znane pułapki").
...
5. **Faza 4 Build — SEKCJA PO SEKCJI.** Najpierw **lista sekcji w kolejności** (kontrakt kompletności). Potem każda sekcja osobno: budujesz komponentami buildera, weryfikujesz zrzutem, dopiero wtedy następna. Codex AUTORUJE (placeholdery `__IMG_*__`), Claude APLIKUJE (biblioteka sekcji + postprocess-fixes + apply-pages). **Nie buduj całej strony naraz i nie porównuj dopiero na końcu** — gdy coś nie zagra, nie wiadomo która sekcja zawiniła. `ready=true` ≠ poprawność wizualna.
